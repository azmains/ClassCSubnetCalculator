...
for entries = 1:turns
   ...
   fprintf("\n")
    %totalHosts is the number of hosts 
    totalHosts= length(storeSubnetIp)*(2^(hostBits) - 2);
    %format subnetmask to fit in table
    formatsubnetMask= sprintf(formatOctets,subnetMask);
    
    %make into table the network data from above
    networkData = table(hostBits,formatsubnetMask,totalHosts);
    hostData=table(hostIps);
    disp(networkData)
    
    %Ask user to view hostIps array
    fprintf("\n")
    fprintf("Would you like to view all host IPs?")
    fprintf("\n")
    
    %Ask user to prompt if they want to view
    requestHost=input("Answer '1' for Yes and '2' for No: ");
    while ~(requestHost ==1 || requestHost == 2)
         option=input("Answer '1' for Yes and '2' for No: ");
    end
    %If requestHost is 1 then hostIps will display
    if requestHost ==1
        for ip = 1:length(hostIps)
            disp(hostIps(ip));
            if mod(ip,14)==0
                fprintf("\n")
            end
